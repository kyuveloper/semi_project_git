...
public class AdminInquiryStatusService {
    ...
    private InquiryDTO convertToDto(AdminInquiryStatus inquiry) {
        ...
        Integer answerCode = inquiry.getAnswerCode();  // null일 수 있는 값
        // answerCode가 null인 경우 기본 값을 설정
        if (answerCode != null && answerCode != 0) {
            inquiryDto.setAnswerCode(answerCode);
        } else {
    ...
    public int getInquiryCount() {
        return (int) adminInquiryStatusRepository.count();
    }
}